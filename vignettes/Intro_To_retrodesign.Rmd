...
to get a standard error of 1. We can use `sim_plot()` to do so, with the first
...
arguement. `retrodesign()` is the original function provided by Gelman & Carlin,
which uses simulation rather than an exact solution to calculate the errors. It's
...
or even get its sign wrong if we're unlucky. Further, these are practical measures to
focus on;
