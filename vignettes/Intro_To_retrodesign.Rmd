...
where we get a standard error of 1. We can use `sim_plot()` to do so, with the first
parameter being our postulated effect size .5, and the second being our
hypothetical standard error of 1.
...
The function arguements here the same as those for `sim_plot()` above.
...
arguement. `retrodesign()` is the original function provided by Gelman & Carlin (2014),
which uses simulation rather than an exact solution to calculate the errors. It's thus
slower, but can work with t-distributions as well.
...
the treatment effect could mean undertreating someone.
...
likely spurious result, is likely to have low power, and a high probability of s/m errors.
...
the causal reasoning seems to be that male fetuses are more likely than female
...
Gelman and Carlin end up looking at .1-1, reasoning that the sex ratios vary
...
By providing a list for `A`, we get a dataframe with a posited effect size,
...
So if we go with a high but reasonable posited effect of 1%, there's a nearly
...
about the true effect size, this study simply seems to small to be informative: Under
most assumptions, we're quite likely to get the sign wrong, and even with a generous
...
the posited effect sizes, to include max out at 10, where the power would be
...
need to worry about type s errors very often unless your power calculations
...
For the majority of this vingette, I've avoided questioning whether we should
...
Type S and M errors are an artifact of the hard threshholding
...
these errors (and many others) fairly easily by abandoning statistical significance as a hard filter entirely.
...
Papers on Type S/Type M Errors:
...
3. **Lu et al's [A note on Type S/M errors in hypothesis testing
Jiannan Lu  Yixuan Qiu  Alex Deng](https://onlinelibrary.wiley.com/doi/full/10.1111/bmsp.12132)
(2018)**: Lu and coauthors go further into the mathamatical properties of Type S/M errors, and prove the closed form solutions implimented in `retrodesign`. 
...
to argue for abandoning NHST all together.
