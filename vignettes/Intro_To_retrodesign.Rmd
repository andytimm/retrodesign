...
In a underpowered example like this, we will hugely overestimate our effect size (by a factor of 4.7x!),
...
(2007), which argued that the most attractive people are more likely to have
...
ggplot(effect_s_pairs, aes(possible_effects, type_s)) + geom_point()
ggplot(effect_m_pairs, aes(possible_effects, type_m)) + geom_point()
```
